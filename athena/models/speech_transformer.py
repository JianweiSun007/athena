...
from ..utils.misc import generate_square_subsequent_mask, generate_memory_subsequent_mask, insert_sos_in_labels
from ..layers.commons import PositionalEncoding
from ..layers.transformer import Transformer, MemoryTransformer
...
class SpeechTransformer3(BaseModel):
    """ Standard implementation of a SpeechTransformer. Model mainly consists of three parts:
    the x_net for input preparation, the y_net for output preparation and the transformer itself
    """
    default_config = {
...
class GateConv(tf.keras.layers.Layer):
    ...
    def __init__(self, filters = 32, **kwargs):
        ...
        self.u = layers.Conv2D(
            ...
            use_bias=False,
            data_format="channels_last",)
    def call(self, cnn_layer_in):
        cnn_layer_out = tf.nn.sigmoid(self.gate3(cnn_layer_in)) * tf.tanh(tf.nn.sigmoid(self.gate1(cnn_layer_in)) * cnn_layer_in + tf.nn.sigmoid(self.gate2(cnn_layer_in)) * tf.nn.tanh(self.u(cnn_layer_in)))
...
class ResLayer(tf.keras.layers.Layer):
    ...
    def call(self, cnn_layer_in):
        # 第一层
        cnn_layer_in = self.gateconv32(cnn_layer_in)
        origin_layer = cnn_layer_in
        cnn_layer_in = tf.nn.tanh(self.conv16(cnn_layer_in))
        # 第二层
        cnn_layer_in = self.gateconv16(cnn_layer_in)
        cnn_layer_in = tf.nn.tanh(self.conv32(cnn_layer_in))
        cnn_layer_in = origin_layer + cnn_layer_in
        origin_layer = cnn_layer_in
        # 第三层
        cnn_layer_in = tf.nn.tanh(self.conv16(cnn_layer_in))
        cnn_layer_in = self.gateconv16(cnn_layer_in)
        ...
        cnn_layer_in = tf.nn.tanh(self.conv32(cnn_layer_in))
        cnn_layer_in = origin_layer + cnn_layer_in
        cnn_layer_out = cnn_layer_in
        return cnn_layer_out

class SpeechMemoryTransformer(BaseModel):
    """ Standard implementation of a SpeechTransformer. Model mainly consists of three parts:
    the x_net for input preparation, the y_net for output preparation and the transformer itself
    """
    default_config = {
    ...
    def call(self, samples, training: bool = None):
        ...
        y = self.y_net(y0, training=training)
        input_length = self.compute_logit_length(samples)
        input_mask, output_mask, output_memory_mask = self._create_masks(x, input_length, y0)
        y, encoder_output = self.transformer(
            x,
            y,
            input_mask,
    ...
    def _create_masks(x, input_length, y):
        ...
        if y is not None:
            output_mask = tf.cast(tf.math.equal(y, 0), tf.float32)
            zeros_mask = tf.zeros(y.shape, tf.float32)
            output_mask = output_mask[:, tf.newaxis, tf.newaxis, :]
            zeros_mask = zeros_mask[:, tf.newaxis, tf.newaxis, :]
            look_ahead_mask, memory_mask = generate_memory_subsequent_mask(tf.shape(y)[1])
            output_mask = tf.maximum(output_mask, look_ahead_mask)
            output_memory_mask = tf.maximum(zeros_mask, memory_mask)
            output_mask.set_shape([None, None, None, None])
            output_memory_mask.set_shape([None, None, None, None])
        return input_mask, output_mask, output_memory_mask
    ...
    def time_propagate(self, history_logits, history_predictions, step, enc_outputs):
        ...
        (encoder_output, memory_mask) = enc_outputs
        step = step + 1
        output_mask, out_memory_mask = generate_memory_subsequent_mask(step)
        # propagate 1 step
        logits = self.y_net(tf.transpose(history_predictions.stack()), training=False)
        logits = self.transformer.decoder(
            logits,
