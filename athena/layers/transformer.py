...
from .attention import MultiHeadAttention,MemoryMultiHeadAttention
...
class MemoryTransformer(tf.keras.layers.Layer):
    """A transformer model. User is able to modify the attributes as needed. The architecture
    is based on the paper "Attention Is All You Need". Ashish Vaswani, Noam Shazeer,
    Niki Parmar, Jakob Uszkoreit, Llion Jones, Aidan N Gomez, Lukasz Kaiser, and
    Illia Polosukhin. 2017. Attention is all you need. In Advances in Neural Information
    ...
    def __init__(
        ...
        if custom_decoder is not None:
            ...
            decoder_layers = [
                MemoryTransformerDecoderLayer(
                    d_model, nhead, dim_feedforward, dropout, activation
                )
                for _ in range(num_decoder_layers)
            ]
            self.decoder = MemoryTransformerDecoder(decoder_layers)

        self.d_model = d_model
        self.nhead = nhead

    def call(self, src, tgt, src_mask=None, tgt_mask=None, attention_mask=None, memory_mask=None,
             return_encoder_output=False, training=None):
        """Take in and process masked source/target sequences.
        ...
        memory = self.encoder(src, src_mask=src_mask, training=training)
        output = self.decoder(
            tgt, memory, tgt_mask=tgt_mask, attention_mask=attention_mask, memory_mask=memory_mask, training=training
        )
        if return_encoder_output:
            return output, memory
        return output
...
class MemoryTransformerEncoder(tf.keras.layers.Layer):
    """TransformerEncoder is a stack of N encoder layers
    ...
    def call(self, src, attention_mask=None, memory_mask=None, training=None):
        """Pass the input through the endocder layers in turn.
        ...
        output = src
        for i in range(len(self.layers)):
            output = self.layers[i](output, attention_mask=attention_mask, memory_mask=memory_mask, training=training)
        return output
...
class MemoryTransformerDecoder(tf.keras.layers.Layer):
    """TransformerDecoder is a stack of N decoder layers
    ...
    def call(self, tgt, memory, tgt_mask=None, attention_mask=None, memory_mask=None, training=None):
        """Pass the inputs (and mask) through the decoder layer in turn.
...
class MemoryTransformerEncoderLayer(tf.keras.layers.Layer):
    """TransformerEncoderLayer is made up of self-attn and feedforward network.
    This standard encoder layer is based on the paper "Attention Is All You Need".
    Ashish Vaswani, Noam Shazeer, Niki Parmar, Jakob Uszkoreit, Llion Jones, Aidan N Gomez,
    Lukasz Kaiser, and Illia Polosukhin. 2017. Attention is all you need. In Advances in
    ...
    def __init__(
        ...
        super().__init__()
        self.self_attn = MemoryMultiHeadAttention(d_model, nhead)
        # Implementation of Feedforward model
        layers = tf.keras.layers
        self.ffn = tf.keras.Sequential(
            [
    ...
    def call(self, src, attention_mask=None, memory_mask=None, training=None):
        ...
        Shape:
            see the docs in Transformer class.
        """
        out = self.self_attn(src, src, src, attention_mask=attention_mask, memory_mask=memory_mask)[0]
        out = self.norm1(src + self.dropout(out, training=training))
        out = self.norm2(out + self.ffn(out, training=training))
...
class MemoryTransformerDecoderLayer(tf.keras.layers.Layer):
    """TransformerDecoderLayer is made up of self-attn, multi-head-attn and feedforward network.
    This standard decoder layer is based on the paper "Attention Is All You Need".
    Ashish Vaswani, Noam Shazeer, Niki Parmar, Jakob Uszkoreit, Llion Jones, Aidan N Gomez,
    Lukasz Kaiser, and Illia Polosukhin. 2017. Attention is all you need. In Advances in
    ...
    def __init__(
        ...
        super().__init__()
        self.attn1 = MemoryMultiHeadAttention(d_model, nhead)
        self.attn2 = MemoryMultiHeadAttention(d_model, nhead)
        # Implementation of Feedforward model
        layers = tf.keras.layers
        self.ffn = tf.keras.Sequential(
            [
    ...
    def call(self, tgt, memory, tgt_mask=None, attention_mask=None, memory_mask=None, training=None):
        ...
        Shape:
            see the docs in Transformer class.
        """
        out = self.attn1(tgt, tgt, tgt, attention_mask=tgt_mask, memory_mask=memory_mask)[0]
        out = self.norm1(tgt + self.dropout1(out, training=training))
        out2 = self.attn2(memory, memory, out, attention_mask=attention_mask)[0]
        #out2 = self.attn2(memory, memory, out, mask=memory_mask)[0]
        out = self.norm2(out + self.dropout2(out2, training=training))
        out = self.norm3(out + self.ffn(out, training=training))
        return out
